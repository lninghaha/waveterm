...
function DurableAwaitingStart({ connected, viewModel, onClose }: DurableAwaitingStartProps) {
        ...
        return (
            <div className="flex flex-col gap-2 max-w-[280px]">
                <div className="font-semibold text-sm flex items-center gap-2 text-secondary whitespace-nowrap">
                    ...
                    {t("Durable Session (Awaiting Connection)")}
                </div>
                <div className="text-xs text-secondary leading-relaxed">
                    {t("Configured for a durable session. The session will start when the connection is established.")}
                </div>
                <LearnMoreButton />
            </div>
        );
    ...
    return (
        <div className="flex flex-col gap-2 max-w-[280px]">
            <div className="font-semibold text-sm flex items-center gap-2 text-secondary whitespace-nowrap">
                <i className="fa-sharp fa-solid fa-shield text-muted" />
                {t("Durable Session (Awaiting Start)")}
            </div>
            <div className="text-xs text-secondary leading-relaxed">
                {t(
...
function DurableStartingContent({ onClose }: DurableStartingContentProps) {
    ...
    return (
        <div className="flex flex-col gap-2 max-w-[280px]">
            <div className="font-semibold text-sm flex items-center gap-2 text-secondary">
                <i className="fa-sharp fa-solid fa-shield text-sky-300" />
                {t("Durable Session (Starting)")}
            </div>
            <div className="text-xs text-secondary leading-relaxed">{t("The durable session is starting.")}</div>
            <LearnMoreButton />
        </div>
    );
}
...
function DurableEndedContent({ doneReason, startupError, viewModel, onClose }: DurableEndedContentProps) {
    ...
    let titleText = t("Durable Session (Ended)");
    let descriptionText = t("The durable session has ended. This block is still configured for durable sessions.");
    const showRestartButton = true;

    if (doneReason === "terminated") {
        titleText = t("Durable Session (Ended, Exited)");
        descriptionText = t(
        ...
        titleText = t("Durable Session (Ended, Lost)");
        descriptionText = t(
        ...
        titleText = t("Durable Session (Failed to Start)");
        descriptionText = t("The durable session failed to start.");
        return (
            <div className="flex flex-col gap-2 max-w-[280px]">
                <div className="font-semibold text-sm flex items-center gap-2 text-secondary">
                    <i className="fa-sharp fa-solid fa-shield text-muted" />
